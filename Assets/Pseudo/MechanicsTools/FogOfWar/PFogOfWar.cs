...
using Pseudo;
using Pseudo.Internal.Mechanics;
...
namespace Pseudo
{
	...
	public class PFogOfWar : PMonoBehaviour
	{
		...
		List<PFogAgent> fogAgents = new List<PFogAgent>();
		...
		void ModifyFog(float[,] alphaMap, PFogAgent fogAgent)
		{
		...
		public void AddAgent(PFogAgent agent)
		{
			fogAgents.Add(agent);
			UpdateFogOfWar = true;
		}

		public void RemoveAgent(PFogAgent agent)
		{
		...
		public List<PFogAgent> GetAgents()
		{
			return fogAgents;
		}

		public PFogAgent GetAgent(int index)
		{
			PFogAgent agent = null;
		...
		public void SetAgents(IList<PFogAgent> fogAgents)
		{
			this.fogAgents = new List<PFogAgent>(fogAgents);
			UpdateFogOfWar = true;
		}
		...
		void Reset()
		{
			CachedRenderer.sprite = HelperFunctions.LoadAssetInFolder<Sprite>("SquareSprite.png", "GraphicsTools");
			CachedRenderer.sharedMaterial = HelperFunctions.LoadAssetInFolder<Material>("FogOfWar.mat", "FogOfWar");
