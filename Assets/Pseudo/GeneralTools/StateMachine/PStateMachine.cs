...
namespace Pseudo
{
	...
	public class PStateMachine : PMonoBehaviour, IStateMachine
	{
		bool isActive;
		...
		void BuildStateDict()
		{
			...
			states = GetComponents<PState>();

			for (int i = 0; i < states.Length; i++)
			{
				IState state = states[i];
				nameStateDict[state.GetType().Name] = state;
				nameStateDict[PStateMachineUtility.FormatState(state.GetType(), state.Layer.GetType())] = state;
			}
		}
		...
		void BuildLayerDict()
		{
			...
			layers = GetComponents<PStateLayer>();
			...
			for (int i = 0; i < layers.Length; i++)
			{
				...
				nameStateDict[PStateMachineUtility.FormatLayer(layer.GetType())] = layer;
			}
		}

		void Reset()
		{
			PStateMachineUtility.CleanUp(null, CachedGameObject);
		}
	}
}
