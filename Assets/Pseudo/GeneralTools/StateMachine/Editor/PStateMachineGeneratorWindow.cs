...
{
	public class PStateMachineGeneratorWindow : CustomWindowBase<PStateMachineGeneratorWindow>
	{
		...
		static PStateMachineGeneratorWindow instance;

		[MenuItem("Pseudo/State Machine Generator")]
		public static PStateMachineGeneratorWindow Create()
		{
		...
		void ShowMachine()
		{
			...
			options.AddRange(PStateMachineUtility.MachineFormattedTypeDict.Keys);
		...
		void ShowInherit()
		{
			...
			options.AddRange(PStateMachineUtility.LayerFormattedTypeDict.Keys);
			...
			if (!initialized || EditorGUI.EndChangeCheck())
			{
				...
				if (PStateMachineUtility.LayerFormattedStateFormattedDict.ContainsKey(Inherit))
				{
					lockedStates = PStateMachineUtility.LayerFormattedStateFormattedDict[Inherit];
		...
		void ShowCallbacks()
		{
			...
			CallbackMask = EditorGUILayout.MaskField(CallbackMask, PStateMachineUtility.FullCallbackNames, GUILayout.MinWidth(150));
		...
		void GenerateLayer()
		{
			...
			if (PStateMachineUtility.LayerFormattedTypeDict.ContainsKey(Inherit))
				layerInherit = PStateMachineUtility.LayerFormattedTypeDict[Inherit].GetName();

			if (PStateMachineUtility.LayerFormattedTypeDict.ContainsKey(SubLayer))
				layerSublayer = PStateMachineUtility.LayerFormattedTypeDict[SubLayer].GetName();
		...
		void GenerateStates()
		{
			...
			for (int i = 0; i < states.Count; i++)
			{
				...
				if (!lockedStates.Contains(state) && !string.IsNullOrEmpty(HelperFunctions.GetAssetPath(stateFileName)))
				{
					Debug.LogError(string.Format("A script named {0} already exists.", stateFileName));
					continue;
				}

				if (PStateMachineUtility.LayerFormattedStateFormattedDict.ContainsKey(Inherit) && PStateMachineUtility.LayerFormattedStateFormattedDict[Inherit].Contains(state))
					stateInherit = PStateMachineUtility.LayerFormattedTypeDict[Inherit].GetName() + state;
		...
		{
			for (int i = 0; i < PStateMachineUtility.FullCallbackNames.Length; i++)
			{
				if ((callbacks & 1 << i) != 0)
				{
					script.Add("	public override void " + PStateMachineUtility.CallbackOverrideMethods[i]);
					script.Add("	{");
					script.Add("		base." + PStateMachineUtility.CallbackBaseMethods[i] + ";");
					script.Add("		");
					script.Add("	}");
					script.Add("	");
				}
